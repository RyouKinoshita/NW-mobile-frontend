...
const SeeOrder = () => {
    ...
    const handleStatusUpdate = async () => {
        try {
            const nextStatus = status === 'Confirmed' ? 'In Storage' : 'Out for Delivery';
            ...
            Alert.alert(
                `${nextStatus} Status Successfully Updated`,
                `The order status has been updated to "${nextStatus}".`,
                [
                    {
                        text: "OK",
                        onPress: () => {
            ...
            Alert.alert(
                ...
                'Something went wrong during the status update process.'
            );
        }
    };
